...
def skew_symmetric(w):
    ...
    wx = torch.stack(
        [torch.stack([O, -w2, w1], dim=-1), torch.stack([w2, O, -w0], dim=-1), torch.stack([-w1, w0, O], dim=-1)],
        dim=-2,
    )
    return wx
...
def exp_r2q(x, y, z, theta):
    lambda_ = torch.sin(theta) / (2.0 * theta + 1e-11)
    qx = lambda_ * x
    qy = lambda_ * y
    qz = lambda_ * z
    qw = torch.cos(theta)
...
def exp_r2q_parallel(r, eps=1e-9):
    ...
    bool_criterion = (theta < eps).unsqueeze(-1).repeat(1, 4)
    assert not torch.isnan(x).any()
    assert not torch.isnan(y).any()
    assert not torch.isnan(z).any()
    q = torch.where(bool_criterion, exp_r2q_taylor(x, y, z, theta), exp_r2q(x, y, z, theta))
...
def log_q2r_parallel(q, eps=1e-9):
    x, y, z, w = q[..., 0], q[..., 1], q[..., 2], q[..., 3]
    theta = torch.sqrt(x ** 2 + y ** 2 + z ** 2)

    bool_theta_0 = theta < eps
    bool_w_0 = torch.abs(w) < eps
    ...
    assert not torch.isnan(r_).any()
    return r_


def log_q2r_taylor_theta_0(w, theta, eps=1e-9):
    assert torch.all(abs(w) > eps)
    lambda_ = 2.0 / w - 2.0 / 3.0 * (theta ** 2) / (w ** 3)
    return lambda_
...
def log_q2r(w, theta):
    lambda_ = 2.0 * (torch.arctan(theta / w)) / theta
    return lambda_
...
def spline_mid_linear(start_pose, end_pose):
    q_start, t_start = se3_2_qt_parallel(start_pose)
    q_end, t_end = se3_2_qt_parallel(end_pose)
    q_start = q_start.squeeze()
    q_end = q_end.squeeze()
    t_start = t_start.squeeze()
    t_end = t_end.squeeze()
    # sample t_vector
    t_t = (t_start + t_end) / 2
    # sample rotation_vector
    q_tau_0 = q_to_Q_parallel(q_to_q_conj_parallel(q_start)).squeeze() @ q_end
    r = log_q2r_parallel(q_tau_0.squeeze(-1)) / 2
    q_t_0 = exp_r2q_parallel(r).squeeze()
    q_t = q_to_Q_parallel(q_start) @ q_t_0
    # convert q&t to RT
    R = q_to_R_parallel(q_t.squeeze(dim=-1))
    t = t_t.unsqueeze(dim=-1)
    pose = torch.cat([R, t], -1)
    # Permute from start1,...,end1,...,start_n,...,end_n to start1,...,start_n,...,end1,...,end_n
    assert not torch.isnan(pose).any()
    return pose


def SplineN_linear(start_pose, end_pose, t_tau, period, delay_time=0, device=None):
    ...
    # Permute from start1,...,end1,...,start_n,...,end_n to start1,...,start_n,...,end1,...,end_n
    # poses = pose_spline.permute((1, 0, 2, 3)).reshape([-1, 3, 4])
    poses = pose_spline
    assert not torch.isnan(poses).any()
    return poses


def SplineN_cubic(pose0, pose1, pose2, pose3, t_tau, period, delay_time=0):
    sample_time = t_tau / (period+delay_time)
    # parallel
