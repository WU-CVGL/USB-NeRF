...
import os, imageio
import torch
from downsample_TUM import downsample
import cv2
...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True, focal = None):
    ...
    imgs = [cv2.imread(f)[..., :3] / 255. for f in imgfiles]
...
def load_llff_data(basedir, factor=1, recenter=True, bd_factor=.75, spherify=False,
    ...
    poses = np.concatenate([poses[:, 1:2, :], -poses[:, 0:1, :], poses[:, 2:, :]], 1)    # -y x z : x y z
